...
export const ChartsPage: React.FC = () => {
  ...
  // Tooltip details
  const [activeTooltip, setActiveTooltip] = useState<{ name: string; value: number; x: number; y: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchRecords = async () => {
      setLoading(true);
      try {
        const res = await fetch(`http://127.0.0.1:8005/api/upload/${selectedDatasetId}/records`);
        if (res.ok) {
          const data = await res.json();
          ...
          if (data.columns.length > 0) {
            ...
            const numericCol = data.columns.find((c: string) => 
              data.data_types[c]?.includes('int') || data.data_types[c]?.includes('float')
            );
  ...
  const explainerData = React.useMemo(() => {
    ...
    const topPct = totalSum > 0 ? ((topItem.value / totalSum) * 100).toFixed(1) : '0';
  ...
  return (
    <div className={`space-y-8 animate-fadeIn max-w-7xl mx-auto ${isFullscreen ? 'fixed inset-0 z-50 bg-stone-950 p-8 overflow-y-auto' : ''}`}>
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Design and execute custom chart figures dynamically over columns dimensions.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="glass-panel p-5 rounded-2xl space-y-4 border border-zinc-900/60">
              <h3 className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Dimension Settings</h3>
              
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <label className="text-[10px] text-zinc-500 font-semibold uppercase">X-Axis Dimension</label>
                  <select
                    value={xAxis}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] text-zinc-500 font-semibold uppercase">Y-Axis Values Measure</label>
                  <select
                    value={yAxis}
                ...
                <div className="space-y-1.5 font-sans">
                  <label className="text-[10px] text-zinc-500 font-semibold uppercase">Visual Layout</label>
                  <div className="grid grid-cols-2 gap-2">
                    {(['bar', 'line', 'pie', 'scatter', 'area'] as const).map(type => (
                      <button
                        key={type}
              ...
              >
                ...
                >
                  ...
                  {[...Array(5)].map((_, i) => {
                    ...
                    return (
                      <g key={i}>
                        <line x1={padding} y1={y} x2={width - padding} y2={y} stroke="#18181b" strokeWidth="0.8" strokeDasharray="3" />
                        <text x={padding - 10} y={y + 4} fill="#52525b" fontSize="8" textAnchor="end" className="font-mono">
                  ...
                  {chartType === 'bar' && chartPoints.map((p, idx) => {
                    ...
                    const barWidth = Math.max(12, (width - 2 * padding) / (chartPoints.length * 1.6));
                    const barHeight = height - padding - coords.y;
                    
                    return (
                      <g key={idx} 
                         className="cursor-pointer group"
                         onMouseEnter={() => setActiveTooltip({ name: p.name, value: p.value, x: coords.x, y: coords.y - 10 })}
                      ...
                      >
                        ...
                        />
                        <text x={coords.x} y={height - padding + 15} fill="#71717a" fontSize="8" textAnchor="middle" className="font-sans font-medium">
                      ...
                      {chartPoints.map((p, idx) => {
                        ...
                        return (
                          <g key={idx}
                             ...
                             onMouseEnter={() => setActiveTooltip({ name: p.name, value: p.value, x: c.x, y: c.y - 10 })}
                             onMouseLeave={() => setActiveTooltip(null)}
                          >
                            <circle cx={c.x} cy={c.y} r="4" fill="#6366f1" stroke="#09090b" strokeWidth="1" />
                            <text x={c.x} y={height - padding + 15} fill="#71717a" fontSize="8" textAnchor="middle">
                              {p.name.length > 8 ? `${p.name.slice(0, 6)}..` : p.name}
                            </text>
                          </g>
                        );
                    ...
                    <>
                      <path
                        d={`${chartPoints.map((p, idx) => {
                          const c = getCoordinates(idx, p.value);
                          return `${idx === 0 ? 'M' : 'L'} ${c.x} ${c.y}`;
                        }).join(' ')} L ${getCoordinates(chartPoints.length - 1, 0).x} ${height - padding} L ${getCoordinates(0, 0).x} ${height - padding} Z`}
                        fill="url(#areaGrad)"
                        opacity="0.25"
                      />
                      <path
                      ...
                      {chartPoints.map((p, idx) => {
                        ...
                        return (
                          <g key={idx}
                             ...
                             onMouseLeave={() => setActiveTooltip(null)}
                          >
                            <circle cx={c.x} cy={c.y} r="3.5" fill="#6366f1" stroke="#09090b" strokeWidth="0.8" />
                            <text x={c.x} y={height - padding + 15} fill="#71717a" fontSize="8" textAnchor="middle">
                              {p.name.length > 8 ? `${p.name.slice(0, 6)}..` : p.name}
                            </text>
                          </g>
                  ...
                  {chartType === 'scatter' && chartPoints.map((p, idx) => {
                    const c = getCoordinates(idx, p.value);
                    return (
                      <g key={idx}
                         className="cursor-pointer"
                         ...
                         onMouseLeave={() => setActiveTooltip(null)}
                      >
                        <circle cx={c.x} cy={c.y} r="5.5" fill="#a855f7" opacity="0.85" stroke="#09090b" strokeWidth="1" />
                        <text x={c.x} y={height - padding + 15} fill="#71717a" fontSize="8" textAnchor="middle">
                          {p.name.length > 8 ? `${p.name.slice(0, 6)}..` : p.name}
                        </text>
                      </g>
                  ...
                  {chartType === 'pie' && (() => {
                    const totalVal = chartPoints.reduce((a, b) => a + b.value, 0);
                    let accumulatedAngle = 0;
                    
                    return chartPoints.map((p, idx) => {
                      const percentage = totalVal > 0 ? p.value / totalVal : 0;
                      ...
                      const midRad = (accumulatedAngle - angle / 2 - 90) * Math.PI / 180;
                      ...
                      return (
                        <g key={idx}
                           className="cursor-pointer"
                           onMouseEnter={() => setActiveTooltip({ name: p.name, value: p.value, x: tx, y: ty })}
                           onMouseLeave={() => setActiveTooltip(null)}
                        >
                          <path
                          ...
                          />
                          {percentage > 0.06 && (
                            <text x={cx + (r * 0.65) * Math.cos(midRad)} y={cy + (r * 0.65) * Math.sin(midRad)} fill="#fff" fontSize="7.5" fontWeight="bold" textAnchor="middle">
                ...
                {activeTooltip && (
                  ...
                  >
                    <span className="font-mono text-zinc-500 font-bold block">{xAxis.toUpperCase()}: {activeTooltip.name}</span>
                    <span className="font-sans font-bold text-indigo-400 block">{yAxis.toUpperCase()}: {activeTooltip.value.toLocaleString()}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {explainerData && (
              <div className="glass-panel p-6 rounded-2xl border border-zinc-900/60 bg-gradient-to-br from-zinc-950/40 to-zinc-950/10 space-y-6">
                <div className="flex items-center gap-2 border-b border-zinc-900/50 pb-3">
                  ...
                  <h3 className="text-xs font-semibold text-zinc-200 uppercase tracking-widest">AI Chart Explainer</h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-zinc-300 leading-relaxed bg-zinc-900/20 p-3 rounded-xl border border-zinc-900/40">
                        This data visualization plots the aggregate values of metric **{yAxis}** grouped across discrete segments of variable **{xAxis}**. A total cumulative measure of **${explainerData.total}** is calculated.
                      </p>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-zinc-300 leading-relaxed bg-zinc-900/20 p-3 rounded-xl border border-zinc-900/40">
                        The highest aggregate concentration is held by segment **'{explainerData.topName}'** contributing **${explainerData.topVal}** ({explainerData.topPct}% share). The lowest aggregate value is tracked in segment **'{explainerData.worstName}'** ({explainerData.worstVal}).
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-zinc-300 leading-relaxed bg-zinc-900/20 p-3 rounded-xl border border-zinc-900/40">
                        High revenue concentration in **'{explainerData.topName}'** indicates robust client demand or wholesale bulk sales, which creates supply chain exposure. A regulatory bottleneck or competitor launch in this segment poses a direct margin threat.
                      </p>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-zinc-300 leading-relaxed bg-zinc-900/20 p-3 rounded-xl border border-zinc-900/40">
                        We recommend diversifying operations by allocating secondary marketing channel budgets towards underperforming segments like **'{explainerData.worstName}'** to establish portfolio equilibrium and minimize concentration risks.
                      </p>
                    </div>
                  </div>
        ...
        <div className="glass-panel p-16 rounded-2xl flex flex-col items-center justify-center text-center space-y-4 min-h-[400px]">
          ...
          <div>
            <h3 className="text-sm font-semibold text-zinc-400">No Datasets Ingested</h3>
            <p className="text-xs text-zinc-600 mt-1 max-w-sm">
              Please ingest a CSV file inside the upload portal first to enable dynamic visual graphs drawing.
            </p>
          </div>
        </div>
      )}
