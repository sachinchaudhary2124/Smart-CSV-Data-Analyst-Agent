...
export const ChatPage: React.FC = () => {
  ...
  const fetchSuggestions = async (id: string) => {
    try {
      const res = await fetch(`http://127.0.0.1:8005/api/upload/${id}/profile`);
      if (res.ok) {
        const profile = await res.json();
        if (profile.suggested_questions && profile.suggested_questions.length > 0) {
  ...
  const initSession = async () => {
    try {
      const sessionRes = await fetch('http://127.0.0.1:8005/api/chat/session', {
        method: 'POST'
      });
      if (sessionRes.ok) {
        ...
        setMessages([
          {
            role: 'assistant',
            content: "Welcome! Choose a dataset from the selector and ask me queries. I'll automatically analyze statistical aggregates, plot diagrams, and construct recommendations."
          }
        ]);
      }

      const datasetsRes = await fetch('http://127.0.0.1:8005/api/upload/recent');
      if (datasetsRes.ok) {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8005/api/chat/query', {
        method: "POST",
      ...
      const nodeTrace = data.workflow_trace || [
        { node: "intent_analyzer", status: "completed", execution_time_ms: 12.0, tool_selected: null },
        { node: "dataset_validator", status: "completed", execution_time_ms: 4.0, tool_selected: null },
        { node: "tool_planner", status: "completed", execution_time_ms: 5.0, tool_selected: data.selected_tool },
        { node: "tool_executor", status: "completed", execution_time_ms: data.execution_time_ms, tool_selected: data.selected_tool },
        { node: "insight_generator", status: "completed", execution_time_ms: 15.0, tool_selected: null },
        { node: "response_formatter", status: "completed", execution_time_ms: 10.0, tool_selected: null },
        { node: "conversation_memory", status: "completed", execution_time_ms: 3.0, tool_selected: null }
      ];
      ...
      for (let i = 0; i < nodeTrace.length; i++) {
        ...
        setThoughtLog(prev => [...prev, `[${traceItem.node}] Executed in ${traceItem.execution_time_ms}ms`]);
      ...
      const typeText = () => {
        if (charIndex < rawText.length) {
          ...
          setActiveReasoning({
            ...
            execution_time_ms: data.execution_time_ms,
            confidence_score: data.confidence_score,
            dataset: datasets.find(d => d.upload_id === selectedDatasetId)?.original_name || "N/A",
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: "Pipeline failed to execute query. Check if the backend daemon is online."
      }]);
      setLoading(false);
      setExecutingNode(null);
      setActiveNodeIndex(null);
  ...
  const handleClearHistory = async () => {
    if (!sessionId) return;
    try {
      const res = await fetch(`http://127.0.0.1:8005/api/chat/session/${sessionId}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        setMessages([{ role: 'assistant', content: "Conversation history cleared." }]);
      ...
      return (
        <div className="mt-4 p-4 rounded-xl bg-zinc-950 border border-zinc-900 w-full space-y-4 max-w-lg">
          ...
          <div className="grid grid-cols-3 gap-2">
            {Object.entries(kpis).slice(0, 3).map(([key, item]: any) => (
              <div key={key} className="bg-zinc-900/60 p-2.5 rounded border border-zinc-900 space-y-0.5">
                <span className="text-[8px] text-zinc-500 font-semibold uppercase block truncate">{item.label}</span>
                <span className="text-[11px] font-bold text-white block truncate">{item.value}</span>
              </div>
            ))}
          </div>

          <div className="text-[9px] text-zinc-400 bg-zinc-900/40 p-2 rounded border border-zinc-900/50">
             To view fully interactive charting, check our <b>Executive Dashboard</b> portal.
          </div>
        </div>
      );
    }
      ...
      <div className="mt-4 p-4 rounded-xl bg-zinc-950 border border-zinc-900 w-full max-w-lg space-y-2">
        ...
        <div className="h-40 w-full flex items-center justify-center">
          <svg width="100%" height="100%" viewBox={`0 0 ${w} ${h}`} className="overflow-visible">
            {chartType === 'bar' && pts.map((pt: any, idx: number) => {
              const coords = getXy(idx, pt.value);
              const barW = (w - 2 * p) / (pts.length * 1.5);
              const barH = h - p - coords.y;
              return (
                <rect key={idx} x={coords.x - barW / 2} y={coords.y} width={barW} height={barH} rx="1.5" fill="#6366f1" opacity="0.8" />
              );
            })}

            {chartType === 'line' && (
              <path
                d={pts.map((pt: any, idx: number) => {
                  const coords = getXy(idx, pt.value);
            ...
            {chartType === 'scatter' && pts.map((pt: any, idx: number) => {
              const coords = getXy(idx, pt.y || pt.value);
              return <circle key={idx} cx={coords.x} cy={coords.y} r="4" fill="#a855f7" />;
            })}
          </svg>
        </div>
      </div>
  ...
  const activeDatasetName = datasets.find(d => d.upload_id === selectedDatasetId)?.original_name || "No CSV loaded";
      ...
      <div className="flex-1 flex flex-col min-w-0 border-r border-zinc-900 bg-background/25">
        ...
        <div className="h-14 border-b border-zinc-900 px-4 flex items-center justify-between bg-zinc-950/60 shrink-0">
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {datasets.map(d => <option key={d.upload_id} value={d.upload_id}>{d.original_name}</option>)}
            </select>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {reasoningOpen ? <ChevronRight size={13} /> : <ChevronLeft size={13} />}
          ...
          {messages.map((message, i) => (
            ...
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border ${
                message.role === 'user' ? 'bg-zinc-900 border-zinc-800 text-zinc-300' : 'bg-indigo-600/10 border-indigo-500/20 text-indigo-400'
              }`}>
              ...
              <div className="space-y-2 max-w-full">
                <div className={`rounded-xl p-3.5 text-xs leading-relaxed border ${
                  message.role === 'user' ? 'bg-indigo-600/10 border-indigo-500/20 text-zinc-100' : 'bg-zinc-900/60 border-zinc-900 text-zinc-300'
                }`}>
                  ...
                  {message.supporting_statistics && (
                    <div className="mt-3 pt-3 border-t border-zinc-900/60 text-[10px] text-zinc-400 space-y-1 overflow-x-auto">
                      <div className="font-semibold text-zinc-300 uppercase tracking-wider text-[9px] mb-1">Supporting Metrics</div>
                  ...
                  {message.chart_data && renderInlineVisual(message.chart_data, message.selected_tool)}
                </div>
                ...
                {message.role === 'assistant' && message.content && (
                  <div className="flex items-center gap-3 text-[10px] text-zinc-500 px-1">
                    ...
                    {message.execution_time_ms !== undefined && (
                      <span className="font-mono text-[9px] ml-auto">Exec: {message.execution_time_ms}ms</span>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="glass-panel p-3.5 rounded-xl border border-zinc-900/80 max-w-sm w-full space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-zinc-300 uppercase tracking-wider">
                    {executingNode ? `Node: ${executingNode}` : "Executing graph..."}
                  </span>
                </div>
              </div>
            </div>
        ...
        {suggestions.length > 0 && !loading && (
          <div className="px-4 py-2.5 flex items-center gap-2 flex-wrap bg-zinc-950/20 border-t border-zinc-900/60 shrink-0 select-none">
            ...
            <span className="text-[9px] text-zinc-500 font-semibold uppercase tracking-wider mr-1">Suggestions:</span>
        ...
        >
          <input
            ...
            placeholder={selectedDatasetId ? `Query analyst over '${activeDatasetName}'...` : "Select a dataset context to ask questions..."}
        ...
        {reasoningOpen && (
          ...
          >
            <div className="flex items-center gap-2 border-b border-zinc-900 pb-3">
              ...
              <h2 className="font-display font-semibold text-xs text-zinc-300 uppercase tracking-widest">AI Reasoning Console</h2>
            </div>

            {/* Workflow Graph Visualizer (Feature 10) */}
            <div className="p-4 rounded-xl bg-zinc-900/20 border border-zinc-900 space-y-4">
              <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Live LangGraph Workflow Visualizer</span>
              ...
              <div className="relative pl-6 space-y-4 border-l border-zinc-800">
                {graphNodesList.map((node, idx) => {
                  ...
                  const styleMap = {
                    ...
                    active: 'border-indigo-500 text-indigo-400 bg-indigo-500/10 shadow-premium animate-pulse scale-105',
                    completed: 'border-emerald-500 text-emerald-400 bg-emerald-500/10'
                  };

                  return (
                    <div key={node.id} className="relative flex items-center justify-between gap-3 transition duration-300">
                      
                      {/* Left Connected Bubble */}
                      <div className={`absolute -left-[35px] w-6 h-6 rounded-full border flex items-center justify-center text-[10px] z-10 font-bold transition-all ${styleMap[status]}`}>
                        {status === 'completed' ? '✓' : node.icon}
                      </div>

                      <div className="space-y-0.5">
                        <span className={`text-[10px] font-semibold block transition-colors ${status === 'active' ? 'text-indigo-400' : status === 'completed' ? 'text-zinc-200' : 'text-zinc-500'}`}>
                          {node.label}
                        </span>
                        {status === 'active' && (
                          <span className="text-[8px] text-zinc-500 uppercase tracking-wider animate-pulse block">Executing...</span>
                        )}
                        {status === 'completed' && activeReasoning?.workflow_trace && (
                          <span className="text-[8px] font-mono text-zinc-600">
                             Latency: {activeReasoning.workflow_trace[idx]?.execution_time_ms.toFixed(1)}ms
                          </span>
                        )}
                      </div>
                    </div>
              ...
              <div className="space-y-6 animate-fadeIn">
                <div className="space-y-3.5 text-[10px] font-mono">
                  <div className="flex items-center justify-between p-2 rounded bg-zinc-900/40 border border-zinc-900">
                    <span className="text-zinc-500 font-sans">Active Target CSV</span>
                    <span className="text-zinc-300 truncate max-w-[120px] font-bold">{activeReasoning.dataset}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded bg-zinc-900/40 border border-zinc-900">
                    <span className="text-zinc-500 font-sans">Identified Goal Intent</span>
                    <span className="text-indigo-400 font-bold">{activeReasoning.detected_intent}</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded bg-zinc-900/40 border border-zinc-900">
                    <span className="text-zinc-500 font-sans">Node Tool Executed</span>
                    <span className="text-cyan-400">{activeReasoning.selected_tool}()</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded bg-zinc-900/40 border border-zinc-900">
                    <span className="text-zinc-500 font-sans">Process Time</span>
                    <span className="text-zinc-300 font-bold">{activeReasoning.execution_time_ms} ms</span>
                  </div>
                  <div className="flex items-center justify-between p-2 rounded bg-zinc-900/40 border border-zinc-900">
                    <span className="text-zinc-500 font-sans">Confidence Index</span>
                    <span className="text-emerald-400 font-bold">{activeReasoning.confidence_score * 100}%</span>
                  </div>
                </div>
                  ...
                  <div className="p-4 rounded-xl bg-zinc-900/30 border border-zinc-900 space-y-3">
                    ...
                    <div className="space-y-3.5 text-[10px]">
                      <div className="space-y-1">
                        <span className="text-zinc-500 font-sans">Observation:</span>
                        <p className="text-zinc-300 leading-normal font-sans">{activeReasoning.insight.observation}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-zinc-500 font-sans">Reasoning:</span>
                        <p className="text-zinc-400 leading-normal font-sans">{activeReasoning.insight.reason}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-zinc-500 font-sans">Margin Impact:</span>
                        <p className="text-zinc-400 leading-normal font-sans">{activeReasoning.insight.business_impact}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-zinc-500 font-sans">Action Plan:</span>
                        <p className="text-indigo-300 leading-normal font-sans font-medium">{activeReasoning.insight.recommendation}</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {!activeReasoning && !loading && (
              <div className="flex flex-col items-center justify-center text-center py-10 text-zinc-600">
                ...
                <p className="text-[10px] max-w-[200px] mt-2 font-sans">
                  Query the AI analyst to trigger planning processes and render node state variables here.
                </p>
              </div>
            )}
          </motion.div>
