...
export const AnalyticsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchAnalysisData = async () => {
      ...
      try {
        ...
        const kpiRes = await fetch(`http://127.0.0.1:8005/api/analytics/overview/${selectedDatasetId}`);
        const healthRes = await fetch(`http://127.0.0.1:8005/api/upload/${selectedDatasetId}/health`);
        ...
        const recRes = await fetch(`http://127.0.0.1:8005/api/upload/${selectedDatasetId}/records`);
        if (recRes.ok) {
  ...
  useEffect(() => {
    ...
    const isNumeric = dtypes[selectedStatColumn]?.includes('int') || dtypes[selectedStatColumn]?.includes('float');
  ...
  const displayMissing = kpis?.health?.missing_percentage !== undefined ? `${kpis.health.missing_percentage}%` : "0%";
  const displayDup = kpis?.health?.duplicate_percentage !== undefined ? `${kpis.health.duplicate_percentage}%` : "0%";
    ...
    <div className="space-y-8 animate-fadeIn max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm font-sans">Audit datasets summaries, preview column types, or examine values tables.</p>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="glass-panel p-5 rounded-xl space-y-3">
              <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest block">Dataset Dimensions</span>
              <div className="text-xl font-bold text-white font-mono">{displayRows} Rows</div>
              <p className="text-xs text-zinc-500">{displayCols} columns loaded in context</p>
            </div>

            <div className="glass-panel p-5 rounded-xl space-y-3">
              <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest block">Aggregate Values Sum</span>
              <div className="text-xl font-bold text-white font-mono truncate">{displayRev}</div>
              <p className="text-xs text-zinc-500">Calculated over primary numeric column</p>
            </div>

            <div className="glass-panel p-5 rounded-xl space-y-3">
              <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest block">Data Mean Average</span>
              <div className="text-xl font-bold text-white font-mono truncate">{displayAvg}</div>
              <p className="text-xs text-zinc-500">Normal distribution center density</p>
            </div>

            <div className="glass-panel p-5 rounded-xl space-y-3">
              <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest block">Sanity Quality Ratios</span>
              <div className="flex gap-4">
                <div>
                  <div className="text-xs font-bold text-zinc-300 font-mono">{displayMissing}</div>
                  <span className="text-[9px] text-zinc-500">Missing cells</span>
                </div>
                <div>
                  <div className="text-xs font-bold text-zinc-300 font-mono">{displayDup}</div>
                  <span className="text-[9px] text-zinc-500">Duplicates</span>
                </div>
              </div>
              <p className="text-xs text-zinc-500">Quality score: {kpis?.health?.health_score}%</p>
            </div>
            ...
            <div className="glass-panel p-5 rounded-xl space-y-4">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-zinc-300 uppercase tracking-wider">Quick Column Statistics Descriptor</span>
                </div>
                <select
                ...
                >
                  {columns.map(c => (
                    <option key={c} value={c}>{c} ({dtypes[c] || 'object'})</option>
                  ))}
                </select>
              </div>
              ...
              {columnStat.isNumeric ? (
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 text-xs font-mono text-center">
                  <div className="p-3 bg-zinc-900/60 border border-zinc-850 rounded">
                    ...
                    <strong className="text-zinc-200 text-sm">{columnStat.sum}</strong>
                  </div>
                  <div className="p-3 bg-zinc-900/60 border border-zinc-850 rounded">
                    <span className="text-[10px] text-zinc-500 block">Mean</span>
                    <strong className="text-indigo-400 text-sm">{columnStat.mean}</strong>
                  </div>
                  <div className="p-3 bg-zinc-900/60 border border-zinc-850 rounded">
                    <span className="text-[10px] text-zinc-500 block">Median</span>
                    <strong className="text-zinc-200 text-sm">{columnStat.median}</strong>
                  </div>
                  <div className="p-3 bg-zinc-900/60 border border-zinc-850 rounded">
                    <span className="text-[10px] text-zinc-500 block">Maximum</span>
                    <strong className="text-emerald-400 text-sm">{columnStat.max}</strong>
                  </div>
                  <div className="p-3 bg-zinc-900/60 border border-zinc-850 rounded">
                    <span className="text-[10px] text-zinc-500 block">Minimum</span>
                    <strong className="text-rose-400 text-sm">{columnStat.min}</strong>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs font-mono">
                  <div className="p-3 bg-zinc-900/60 border border-zinc-850 rounded text-center">
                    <span className="text-[10px] text-zinc-500 block">Unique Cards Cardinality</span>
                    <strong className="text-zinc-200 text-sm">{columnStat.unique} unique values</strong>
                  </div>
                  <div className="p-3 bg-zinc-900/60 border border-zinc-850 rounded text-center">
                    <span className="text-[10px] text-zinc-500 block">Top Frequency Mode</span>
                    <strong className="text-indigo-400 text-sm truncate block max-w-full" title={columnStat.top}>{columnStat.top}</strong>
                  </div>
                  <div className="p-3 bg-zinc-900/60 border border-zinc-850 rounded text-center">
                    <span className="text-[10px] text-zinc-500 block">Mode Frequency Share</span>
                    <strong className="text-zinc-200 text-sm">{columnStat.topFrequency} times</strong>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="glass-panel rounded-xl border border-zinc-900 overflow-hidden space-y-4">
            ...
            <div className="px-5 py-4 border-b border-zinc-900/60 bg-zinc-950/20 flex flex-col sm:flex-row justify-between items-center gap-4">
              ...
              <div className="flex items-center gap-4 text-xs text-zinc-500">
                <span>Total records matching: <strong className="text-zinc-300 font-mono">{filteredRecords.length}</strong></span>
                <select
                  value={pageSize}
              ...
              <table className="w-full border-collapse text-[11px] font-mono text-left">
                  ...
                  <tr className="text-zinc-400">
                    {columns.map(col => (
                      ...
                      >
                        ...
                        <div className="text-[9px] text-zinc-600 font-mono font-normal tracking-tight pt-0.5">{dtypes[col] || 'object'}</div>
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {paginatedRecords.length === 0 ? (
                    <tr>
                      <td colSpan={columns.length} className="p-8 text-center text-zinc-600 italic">No records matching searches.</td>
                    </tr>
                  ) : (
                    paginatedRecords.map((row, rIdx) => (
                      <tr key={rIdx} className="border-b border-zinc-900/40 hover:bg-zinc-900/20 text-zinc-300">
                        {columns.map(col => (
                          <td key={col} className="p-3 border-r border-zinc-900/40 truncate max-w-[150px]">
        ...
        <div className="glass-panel p-16 rounded-2xl flex flex-col items-center justify-center text-center space-y-4 min-h-[400px]">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-600 mt-1 max-w-sm">
              Ingest a CSV dataset inside the upload portal first to activate stats calculations and tables exploration.
            </p>
          </div>
        </div>
      )}
