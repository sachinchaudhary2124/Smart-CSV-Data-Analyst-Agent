...
export const ReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchGeneratedSummary = async () => {
      setLoading(true);
      try {
        const res = await fetch(`http://127.0.0.1:8005/api/report/generate/${selectedDatasetId}`);
        if (res.ok) {
          const data = await res.json();
          setReport(data);
        }
  ...
  const handleExportPDF = () => {
    ...
    window.open(`http://127.0.0.1:8005/api/report/pdf/${selectedDatasetId}`, '_blank');
  };

  const handleExportPPTX = () => {
    if (!selectedDatasetId) return;
    window.open(`http://127.0.0.1:8005/api/report/pptx/${selectedDatasetId}`, '_blank');
  };
    ...
    <div className="space-y-8 animate-fadeIn max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-zinc-900 pb-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Review compiled business summaries, key opportunities, and download structured documents.</p>
        </div>
      ...
      <div className="glass-panel p-4 rounded-2xl flex items-center gap-3 border border-zinc-900/60 max-w-md">
        ...
        <span className="text-xs text-zinc-400 font-semibold">Target Dataset:</span>
        <select
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6 lg:col-span-1">
            <div className="glass-panel p-5 rounded-2xl border border-zinc-900/50 space-y-6">
              <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Analysis Health Summary</h3>
              
              {/* Health Score Gauge */}
              <div className="flex items-center justify-between gap-4 bg-zinc-900/20 p-4 rounded-xl border border-zinc-900/40">
                <div className="space-y-1">
                  <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">Business Health</span>
                  <div className="text-2xl font-bold text-white">{report.business_health_score}%</div>
                </div>
              ...
              <div className="flex items-center justify-between gap-4 bg-zinc-900/20 p-4 rounded-xl border border-zinc-900/40">
                <div className="space-y-1">
                  <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">AI Confidence Score</span>
                  <div className="text-2xl font-bold text-white">{report.confidence_score}%</div>
                </div>
              ...
              <div className="flex items-center justify-between gap-4 bg-zinc-900/20 p-4 rounded-xl border border-zinc-900/40">
                <div className="space-y-1">
                  <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">Action Priority</span>
                  <div className="text-2xl font-bold text-white">{report.priority_score} <span className="text-xs text-zinc-500 font-medium">/ 100</span></div>
                </div>
            ...
            <div className="glass-panel p-6 rounded-2xl border border-zinc-900/50 bg-zinc-950/20 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {report.key_findings.map((item, idx) => (
                    <div key={idx} className="p-3.5 rounded-xl bg-zinc-900/20 border border-zinc-900/50 text-[11px] text-zinc-300 leading-relaxed">
                      {item}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2.5">
                    {report.business_risks.map((item, idx) => (
                      <div key={idx} className="flex gap-2.5 p-3 rounded-xl bg-rose-500/5 border border-rose-500/10 text-[11px] text-zinc-300 leading-normal">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2.5">
                    {report.growth_opportunities.map((item, idx) => (
                      <div key={idx} className="flex gap-2.5 p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10 text-[11px] text-zinc-300 leading-normal">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2.5">
                  {report.recommended_actions.map((act, idx) => (
                    <div key={idx} className="flex items-start gap-3.5 p-4 rounded-xl bg-zinc-900/40 border border-zinc-900/60">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-[11px] text-zinc-400 leading-relaxed"><span className="text-indigo-400 font-semibold">Action: </span>{act.action}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-3">
                ...
                <ul className="list-disc pl-5 text-[11px] text-zinc-300 space-y-2 leading-relaxed">
                  {report.future_predictions.map((p, idx) => <li key={idx}>{p}</li>)}
                </ul>
              </div>
        ...
        <div className="glass-panel p-12 rounded-2xl flex flex-col items-center justify-center text-center space-y-4">
          ...
          <p className="text-xs text-zinc-500">No active dataset selected. Pick an ingested CSV from the selector to view reports.</p>
        </div>
      )}
