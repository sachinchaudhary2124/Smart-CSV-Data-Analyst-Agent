...
export const HistoryPage: React.FC = () => {
  ...
  const fetchHistory = async (search?: string) => {
    ...
    try {
      const url = search 
        ? `http://127.0.0.1:8005/api/chat/history/list?search=${encodeURIComponent(search)}`
        : 'http://127.0.0.1:8005/api/chat/history/list';
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`http://127.0.0.1:8005/api/chat/history/item/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        setHistoryItems(prev => prev.filter(item => item.history_id !== id));
  ...
  const handleExport = (format: 'csv' | 'json') => {
    window.open(`http://127.0.0.1:8005/api/chat/history/export/download?format=${format}`, '_blank');
  };
    ...
    <div className="space-y-8 animate-fadeIn max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Review, analyze, and export past conversational executions logged in the session.</p>
        </div>
      ...
      {loading ? (
                ...
                >
                  ...
                  >
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="space-y-1 flex-1">
                        <h4 className="text-xs font-semibold text-zinc-100 line-clamp-1">{item.question}</h4>
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-zinc-500 font-medium">
                          <span>Intent: <b className="text-indigo-400 font-bold">{item.detected_intent}</b></span>
                          <span>Tool: <code className="bg-zinc-900 px-1 py-0.5 rounded text-zinc-400">{item.selected_tool}</code></span>
                          <span>Time: <span className="font-mono text-zinc-400">{item.execution_time_ms.toFixed(1)}ms</span></span>
                          <span>Logged: <span className="font-mono">{formatDate(item.timestamp)}</span></span>
                          {item.has_chart && <span className="text-emerald-500 font-bold uppercase tracking-wider text-[8px] bg-emerald-500/10 px-1.5 py-0.5 rounded">Chart</span>}
                        </div>
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-5 pt-2 border-t border-zinc-900/60 bg-zinc-950/20 space-y-3">
                      <h5 className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">Assistant Response</h5>
        ...
        <div className="glass-panel p-16 rounded-2xl flex flex-col items-center justify-center text-center space-y-4 min-h-[300px]">
          ...
          <div>
            <h3 className="text-sm font-semibold text-zinc-400">No Query History Logged</h3>
            <p className="text-xs text-zinc-600 mt-1 max-w-sm">
              Conversations logs will automatically index here. Try asking a summary question in the AI Chat window first.
            </p>
          </div>
        </div>
      )}
